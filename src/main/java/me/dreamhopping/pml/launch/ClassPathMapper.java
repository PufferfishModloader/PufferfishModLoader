...
public class ClassPathMapper {
    ...
    public ClassPathData scanClassPath() throws IOException {
        ...
        for (String s : entries) {
            ...
            EntryType type = getEntryType(file, hashes);
            switch (type) {
                case MINECRAFT:
                    base.add(file);
                    break;
    ...
    private EntryType getEntryType(File file, List<String> hashes) throws IOException {
        ...
        if (file.isDirectory()) return EntryType.EXTRA;
